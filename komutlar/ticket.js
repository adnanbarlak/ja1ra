    ...
    client.channels
      ...
      .send(
        new Discord.MessageEmbed()
          .setTitle(`byhack`)
          .setFooter(`byhack`, client.user.avatarURL())
          .setColor("GREEN")
      ...
      .then(m => {
        ...
        aç.on("collect", async reaction => {
          ...
          const sd = await data.fetch(`ass.author`);
          const mta = await data.fetch(`kar.author`);
          data.add(`numara.${message.guild.id}`, 1);
          const as = await data.fetch(`numara.${message.guild.id}`);
                    if(mta > 2){
      return author.send("Ticket Açma Sınırına Ulaşmıssınız Artık Ticket Açamazsınız")
    }
